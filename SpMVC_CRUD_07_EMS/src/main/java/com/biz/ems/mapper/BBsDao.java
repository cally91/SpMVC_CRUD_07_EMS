...
public interface BBsDao {
	...
	@Select(" SELECT * " + " FROM tbl_bbs " + " WHERE bbs_main_seq = 0 " // 원글 목록만 가져오기
			+ " ORDER BY bbs_date DESC, bbs_time DESC ")
	@Results(value = { @Result(property = "bbs_seq", column = "bbs_seq"),
			@Result(property = "bbs_rep_list", column = "bbs_seq", javaType = List.class, many = @Many(select = "getRepList")) })
	public List<BBsDto> selectAll();

	/*
	 * @SelectKey를 사용하여 insert 이전에 미리 bbs_seq 값을 생성해 두면 Service나 Controller에서 bbsVO로
	 * 부터 bbs_seq를 추출할수 있다.
	 */
	...
	@Results(value = { @Result(property = "bbs_seq", column = "bbs_seq"),
			@Result(property = "bbs_files", column = "bbs_seq", javaType = List.class, many = @Many(select = "getFileList")) })
